...
export default function EmailCampaignView({ showPageHeader = true }) {
  ...
  useEffect(() => {
    let cancelled = false;
    getActiveProviderForChannel(MARKETING_CHANNELS.EMAIL).then((res) => {
      if (cancelled) return;
  ...
  const handleSendNow = useCallback(async () => {
    ...
    setSaving(false);
  }, [canSend, campaignName, subject, body, recipientGroup, audienceMode, selectedIds, recipientCount]);
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm p-6">
            ...
            <div className="space-y-4">
              ...
              <div className="border-t border-gray-200 dark:border-gray-700 pt-4">
                ...
                <div className="bg-gray-50 dark:bg-gray-900 rounded-lg p-4 border border-gray-200 dark:border-gray-700 min-h-[6rem]">
                  <p className="text-xs text-gray-500 dark:text-gray-400 mb-2">Subject: {subject || '—'}</p>
                  <p className="text-sm text-gray-700 dark:text-gray-300 whitespace-pre-wrap">
                    {body || <span className="text-gray-400 dark:text-gray-500">Email preview will appear here.</span>}
                  </p>
                </div>
              </div>
