/**
 * Deletes a client message log entry.
 * POST body: { userId, messageId, organizationId? }
 */
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { error: deleteErr } = await supabaseAdmin.from('client_messages').delete().eq('id', messageId);

    if (deleteErr) {
      console.error('[delete-client-message]', deleteErr);
      return res.status(500).json({ error: 'Failed to delete message' });
    }

    return res.status(200).json({ ok: true });
  } catch (err) {
    console.error('[delete-client-message]', err);
    return res.status(500).json({ error: 'Failed to delete message' });
  }
}
