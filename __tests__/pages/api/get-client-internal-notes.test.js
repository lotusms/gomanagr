...
describe('get-client-internal-notes API', () => {
  ...
  it('returns 200 with notes when organizationId set and user is member', async () => {
          ...
          eq: () => ({
            eq: () => ({
              order: () => ({
                order: () =>
                  Promise.resolve({
                    data: [{ id: 'n2', content: 'Org note', client_id: 'c1' }],
                    error: null,
                  }),
              }),
            }),
    ...
    expect(res.status).toHaveBeenCalledWith(200);
    expect(res.json).toHaveBeenCalledWith({
      notes: [{ id: 'n2', content: 'Org note', client_id: 'c1' }],
    });
  });
  ...
  it('returns 200 with single note when noteId is set', async () => {
        ...
        eq: () => ({
          eq: () => ({
            is: () => ({
              eq: () =>
                Promise.resolve({
                  data: [{ id: 'n-one', content: 'One', client_id: 'c1' }],
                  error: null,
                }),
            }),
          }),
    ...
    await handler(
      {
        method: 'POST',
        body: { userId: 'u1', clientId: 'c1', noteId: 'n-one' },
      },
      res
    );
    expect(res.status).toHaveBeenCalledWith(200);
    expect(res.json).toHaveBeenCalledWith({
      note: { id: 'n-one', content: 'One', client_id: 'c1' },
    });
  });
  ...
  it('returns 404 when noteId set but note not found', async () => {
    ...
    expect(res.json).toHaveBeenCalledWith({ error: 'Internal note not found' });
  });
  ...
  it('returns 500 when data query errors', async () => {
    ...
    expect(res.json).toHaveBeenCalledWith({ error: 'Failed to load internal notes' });
  });
