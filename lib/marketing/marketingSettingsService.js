 ...
 * @returns {Promise<void>} Resolves on success; rejects with error message if API is used and fails.
 */
export async function saveMarketingSettings(settings, userId) {
  ...
  if (userId && typeof userId === 'string' && userId.trim()) {
    const body = { userId: userId.trim(), ...toStore };
    ...
    // Do not cache full config in localStorage after API save; it may contain plaintext secrets.
    // Next load will fetch masked values from API and cache those.
    return;
  }
