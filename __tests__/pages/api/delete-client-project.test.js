/**
 * Unit tests for delete-client-project API.
...
describe('delete-client-project API', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    mockFrom.mockImplementation((t) => {
      if (t === 'client_projects') {
        const selectChain = { eq: () => ({ limit: () => ({ single: () => Promise.resolve({ data: { id: 'proj-1', user_id: 'u1', organization_id: null }, error: null }) }) }) };
  ...
  it('returns 405 for unsupported method', async () => {
    const handler = (await import('@/pages/api/delete-client-project')).default;
    const res = mockRes();
  ...
  it('returns 503 when Supabase unavailable', async () => {
    ...
    const handler = (await import('@/pages/api/delete-client-project')).default;
    const res = mockRes();
    await handler({ method: 'POST', body: { userId: 'u1', projectId: 'proj-1' } }, res);
  ...
  it('returns 403 when project does not belong to user', async () => {
    mockFrom.mockImplementation((t) => {
      if (t === 'client_projects') {
        return { select: () => ({ eq: () => ({ limit: () => ({ single: () => Promise.resolve({ data: { id: 'proj-1', user_id: 'other', organization_id: null }, error: null }) }) }) }) };
      }
      return {};
    });
    const handler = (await import('@/pages/api/delete-client-project')).default;
