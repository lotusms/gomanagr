/**
 * Unit tests for get-task API.
 * POST only; 503, 400 (missing userId, organizationId, or taskId), 403, 500, 404, 200 with task.
 */
...
describe('get-task API', () => {
          ...
          select: () => ({
            eq: () => ({
              eq: () => ({
                maybeSingle: () =>
                  Promise.resolve({
                    data: { id: 'task-1', title: 'Task One', organization_id: 'org-1' },
                    error: null,
                  }),
              }),
            }),
  ...
  it('returns 405 for non-POST', async () => {
    const handler = (await import('@/pages/api/get-task')).default;
    const res = mockRes();
  ...
  it('returns 500 when tasks query errors', async () => {
    mockFrom.mockImplementation((t) => {
      if (t === 'org_members') {
        return {
          select: () => ({
