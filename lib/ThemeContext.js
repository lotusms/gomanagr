...
export const ThemeProvider = ({ children }) => {
  ...
  // Initialize: Ensure dark mode is NOT applied on initial load (before user data is fetched)
  // This prevents dark mode from being applied to anonymous users or before we know the user's preference
  useEffect(() => {
    if (typeof document !== 'undefined' && !currentUser) {
      // Remove dark mode immediately if no user is logged in
      document.documentElement.classList.remove('dark');
    }
  }, []); // Run only once on mount

  // Load palettes and set theme: default for anonymous, from user account when logged in. Never apply default for a logged-in user until we've fetched their account (so selectedPalette always wins).
  useEffect(() => {
    let cancelled = false;
    setThemeReady(false);
    ...
    import('./themes').then((module) => {
      ...
      if (currentUser) {
        ...
        applyDefault();
        // Default to light mode for anonymous users (ensure dark mode is removed)
  ...
  // Watch for auth state changes and remove dark mode when user logs out
  useEffect(() => {
    if (!currentUser && typeof document !== 'undefined') {
      // User logged out - ensure dark mode is removed
      document.documentElement.classList.remove('dark');
    }
  }, [currentUser]);

  // Listen for theme mode updates from ThemeSettings
  useEffect(() => {
    if (!currentUser?.uid) return;

    const handleThemeModeUpdate = async (event) => {
      // Check if this is a theme mode update
    ...
    // Listen for both 'useraccount' and 'useraccount-updated' events
  ...
  const applyPaletteColors = (paletteId, palettes) => {
    ...
    // If the early script in _document already applied this palette, skip DOM update to avoid flash/repaint
    ...
    // Persist to cookie so _document can apply this theme before first paint on next load (avoids flash)
    if (typeof document !== 'undefined') {
      document.cookie = `gomanagr_palette=${encodeURIComponent(paletteId)};path=/;max-age=31536000;SameSite=Lax`;
    }
  };
