/**
 * Returns client meeting note entries. POST body: { userId, clientId, organizationId?, noteId? }
 */
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (error) {
      console.error('[get-client-meeting-notes]', error);
      return res.status(500).json({ error: 'Failed to load meeting notes' });
    }

    if (noteId) {
      const one = Array.isArray(data) ? data[0] : data;
      if (!one) return res.status(404).json({ error: 'Meeting note not found' });
      return res.status(200).json({ note: one });
    }

    return res.status(200).json({ notes: data || [] });
  } catch (err) {
    console.error('[get-client-meeting-notes]', err);
    return res.status(500).json({ error: 'Failed to load meeting notes' });
  }
}
