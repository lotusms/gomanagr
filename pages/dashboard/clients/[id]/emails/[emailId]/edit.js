...
export default function EditClientEmailPage() {
  ...
  useEffect(() => {
    if (!orgReady || !currentUser?.uid || !clientId || !emailId) return;
  ...
  }, [orgReady, currentUser?.uid, clientId, emailId, organization?.id]);

  const backUrl = `/dashboard/clients/${clientId}/edit?tab=communication`;

  if (!currentUser?.uid || !clientId || !emailId) {
    return null;
  }
  ...
  if (loading) {
    return (
      <>
        <Head>
          <title>Edit email - GoManagr</title>
        </Head>
  ...
  if (notFound || !email) {
    return (
      <>
        <Head>
          <title>Email not found - GoManagr</title>
        </Head>
  ...
  return (
    <>
      <Head>
        <title>Edit email - GoManagr</title>
        <meta name="description" content="Edit this email log entry" />
      </Head>
