...
-- Stores encrypted third-party provider config per organization (Stripe, Twilio, Mailchimp, Resend, SMTP).
-- Access via service role only; RBAC enforced in API layer. No RLS on this table.
-- Note: Migration 072 adds 'smtp' to the provider CHECK for existing DBs.

CREATE TABLE IF NOT EXISTS organization_integrations (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  organization_id UUID NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
  provider TEXT NOT NULL CHECK (provider IN ('stripe', 'twilio', 'mailchimp', 'resend', 'smtp')),
