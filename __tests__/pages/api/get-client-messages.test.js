...
describe('get-client-messages API', () => {
  beforeEach(() => {
    jest.clearAllMocks();
        ...
        eq: () => ({
          eq: () => ({
            is: () => ({
              order: () =>
                Promise.resolve({
                  data: [{ id: 'msg-1', body: 'Hello', client_id: 'c1' }],
                  error: null,
                }),
            }),
          }),
  ...
  it('returns 200 with messages array when query succeeds', async () => {
    const handler = (await import('@/pages/api/get-client-messages')).default;
