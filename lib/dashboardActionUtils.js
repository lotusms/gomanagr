 ...
 */
export function buildFollowUps(data, clientNameById = {}) {
  const items = [];
  ...
  (data.invoices || []).forEach((inv) => {
    ...
    const reason = days < 0 ? 'Invoice overdue' : 'Invoice due';
    items.push({
      id: `inv-${inv.id}`,
      type: 'invoice',
      reason,
 ...
 */
export function buildRecentlyUpdated(data, clientNameById = {}, limit = 5) {
  const entries = [];
